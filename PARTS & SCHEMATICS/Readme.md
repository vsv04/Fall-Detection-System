...
The Raspberry Pi 4 board can be purchased from [CanaKit](https://www.canakit.com/raspberry-pi-4-2gb.html). You can also purchase a [complete starter kit](https://www.canakit.com/raspberry-pi-4-complete-starter-kit-official-case.html) that comes with a Raspberry Pi 4, case, power supply, keyboard, mouse and SD card. 
...
The FLIR Lpeton thermal camera can be interfaced with the Raspberry Pi using SPI video protocol and I2C control protocol. The breakout board pins are connected to the GPIO pins on the Raspberry Pi using jumper wires. A comprehensive GPIO pinout diagram for the Raspberry Pi is available at [pinout.xyz](https://pinout.xyz/). </br>
```
CS -> Pin 24, CE0
MOSI -> Pin 19, MOSI
MISO -> Pin 21, MISO
